...
func main() {
	// Remote API URL.
	remoteAPIURL := "https://www.advancionsciences.com/resources/"
	...
	for _, doc := range finalList {
		...
		downloadURLSlice = appendToSlice(downloadURLSlice, doc)
	}
...
func extractPDFUrls(input string) []string {
	...
	for _, match := range matches {
		if len(match) > 1 {
			pdfUrls = append(pdfUrls, strings.ToLower(match[1]))
		}
	}
	return pdfUrls
}
